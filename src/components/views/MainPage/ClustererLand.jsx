...
function ClustererLand ({ isMobile=false, SelectLandByPNU, setRegLandData }) {
    ...
    function handleRegionPageOpen(idx) {
        const land = {
            ...
            addr: selectLand.region[idx],
            pnu: selectLand.pnu[idx],
        };
        ...
        return (
            <Container>
                <Content>
                    ...
                    {selectLand.pnu.length === 0 ? (
                            <NoResultsText>조건에 맞는 검색 결과가 없습니다.</NoResultsText>
                        ) : (
                            selectLand.pnu.map((pnu, i) => (
                                <RegionButton onClick={() => handleRegionPageOpen(i)}>
                                    <RegionContent>
                                        <AddrText>{selectLand.region[i]}</AddrText>
                                        <div style={{display: "flex-direction"}}>
                                            <RegionPriceContent>
                                                <RegionPriceTitle>토지 예측가</RegionPriceTitle><br/>
                                                {selectLand.price[i] != 0 ? Math.floor(selectLand.price[i]).toLocaleString('ko-KR') : "-"}
                                            </RegionPriceContent>
                                            <RegionPriceContent style={selectLand.ratio[i] > 100 ? {color: palette.redM, border:"0"} : {color: palette.blueM, border:"0"}}>
                                                <RegionPriceTitle>공시지가의</RegionPriceTitle><br/>
                                                {selectLand.ratio[i] != 0 ? selectLand.ratio[i] : "-"}%
                                            </RegionPriceContent>
                                        </div>
                                    </RegionContent>
                                </RegionButton>
