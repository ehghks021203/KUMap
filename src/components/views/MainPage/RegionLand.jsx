...
import { setSelectLand } from '../../../actions/globalValues';
import { ZOOM_LEVELS, LAND_TYPES } from "../../../enums";
import palette from '../../../lib/styles/colorPalette';

function RegionLand ({ isMobile=false, SelectLandByPNU, setRegLandData }) {
    ...
    const handleLandPageOpen = (land) => {
        SelectLandByPNU(land.lat, land.lng, land.pnu, land.addr);
    }
    ...
    if (!isMobile) {
        return (
            <Container>
                    ...
                    {selectLand.type === LAND_TYPES.REGION_LAND_UNLOAD ? (
                        <Content>
                            <Loading/>
                        </Content>
                    ) : (
                        <Content>
                            <CenterAddrText>{selectLand.addr}</CenterAddrText>
                            {selectLand.avg_predict_land_price != 0 ? (
                                <div style={{display:"flex", flexDirection: "column", alignItems: "center"}}>
                                    <LandInfoMiniText>예측된 토지: 총 {selectLand.total_land_count}개</LandInfoMiniText><br/>
                                
                                    <LandInfoMiniText>평균 토지 예측가: </LandInfoMiniText>
                                    <LandInfoPriceText style={selectLand.price_ratio > 100 ? {color: palette.redM} : {color: palette.blueM}}>{Math.floor(selectLand.avg_predict_land_price).toLocaleString('ko-KR')}원/m²당</LandInfoPriceText>
                                    <div>
                                        <LandInfoMiniText>공시지가의 </LandInfoMiniText>
                                        <LandInfoMiniText style={selectLand.price_ratio > 100 ? {color: palette.redM} : {color: palette.blueM}}>{selectLand.price_ratio}%</LandInfoMiniText>
                                    </div>
                                </div>
                                ) : (
                                    <div style={{display:"flex", flexDirection: "column", alignItems: "center"}}>
                            ...
                            <br/>
                            {selectLand.land.map((land, i) => (
                                <RegionButton onClick={() => handleLandPageOpen(land)}>
                                    <RegionContent>
                                        <AddrText>{land.addr.address}</AddrText>
