...
import { fetchUserLandLikeListData, fetchUserPropertyListingsData } from "../../utils/api";
...
const Sidebar = ({ isMobile=false, width=320, LoadLand }) => {
    ...
    useEffect(() => {
            if (userLoginStatus) {
                // 관심목록 불러오기
                fetchUserLandLikeListData({ email: currUser.email })
                .then(response => {
                    setFavoriteLandList(response.data.land_like);
                    console.log(response.data.land_like);
                })
                .catch(error => {
                ...
                fetchUserPropertyListingsData({ email: currUser.email })
                .then(response => {
                    setMySaleList(response.data.land_for_sale);
                })
                .catch(error => {
                    if (axios.isCancel(error)) {
                        console.error("Request canceled", error.message);
